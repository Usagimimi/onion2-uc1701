...
int spiCheckDevice(int busNum, int devId)
{
	...
	// open the file
	status	= _spiGetFd(busNum, devId, &fd);
...
int spiRegisterDevice (spiParams *params)
{
	...
	status	= spiCheckDevice(params->busNum, params->deviceId);

	if (status == EXIT_FAILURE) {
		// device file does not exist - register the spi device
		status	= _spiRegisterDevice(params);
		...
		printf_dbg("> SPI device already available\n");
	}
...
int spiSetupDevice (spiParams *params)
{
	...
	if (status == EXIT_SUCCESS) {
		printf_dbg("> Initializing SPI parameters...\n");
		printf_dbg("  > Set SPI mode:       0x%x\n", params->modeBits);
		printf_dbg("  > Set bits per word:  %d\n", params->bitsPerWord);
		printf_dbg("  > Set max speed:      %d Hz (%d KHz)\n", params->speedInHz, (params->speedInHz)/1000);


		// set the SPI mode
		ret = ioctl(fd, SPI_IOC_WR_MODE32, &(params->modeBits) );
		if (ret == -1) {
			printf_dbg("ERROR: Cannot set SPI mode 0x%02x\n", params->modeBits);
			return EXIT_FAILURE;
		}
		...
		if (ret == -1) {
			printf_dbg("ERROR: Cannot set %d bits per word\n", params->bitsPerWord);
			return EXIT_FAILURE;
		}
		...
		if (ret == -1) {
			printf_dbg("ERROR: Cannot set max speed %d Hz\n", params->speedInHz);
			return EXIT_FAILURE;
		}
...
int spiTransfer(spiParams *params, uint8_t *txBuffer, uint8_t *rxBuffer, int bytes)
{
	...
	if (status == EXIT_SUCCESS) {
		...
		if (res < 1) {
			// send failed
			printf_dbg("ERROR: SPI transfer failed\n");
			*rxBuffer 	= 0;
			status	= EXIT_FAILURE;
		}
...
int _spiGetFd(int busNum, int devId, int *devHandle)
{
	...
	if ( (*devHandle = open(pathname, O_RDWR)) < 0) {
		printf_dbg("ERROR: could not open sysfs device '%s'\n", pathname);
		return 	EXIT_FAILURE;
	}
...
int _spiRegisterDevice (spiParams *params)
{
	...
	printf_dbg("> Registering SPI device:\n");
	printf_dbg("  > bus%d, device id: %d\n", params->busNum, params->deviceId);
	printf_dbg("   > SCK:  GPIO%d\n", params->sckGpio);
	printf_dbg("   > MOSI: GPIO%d\n", params->mosiGpio);
	printf_dbg("   > MISO: GPIO%d\n", params->misoGpio);
	printf_dbg("   > CS:   GPIO%d\n", params->csGpio);
	printf_dbg("  > SPI Mode:      %d\n", params->mode);
	printf_dbg("  > Max Frequency: %d Hz (%d kHz)\n", params->speedInHz, (params->speedInHz)/1000);
...
static void hex_dump(const void *src, size_t length, size_t line_size, char *prefix)
{
        ...
        printf_dbg("%s | ", prefix);
        while (length-- > 0) {
                printf_dbg("%02X ", *address++);
                if (!(++i % line_size) || (length == 0 && i % line_size)) {
                        if (length == 0) {
                                while (i++ % line_size)
                                        printf_dbg("__ ");
                        }
                        printf_dbg(" | ");  /* right close */
                        while (line < address) {
                                c = *line++;
                                printf_dbg("%c", (c < 33 || c == 255) ? 0x2E : c);
                        }
                        printf_dbg("\n");
                        if (length > 0)
                                printf_dbg("%s | ", prefix);
